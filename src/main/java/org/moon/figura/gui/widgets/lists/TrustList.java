...
public class TrustList extends AbstractList {
    ...
    public void updateList(TrustContainer container) {
        ...
        //defaults
        trusts.put(new FiguraText(), generateWidgets(container, Trust.DEFAULT, FiguraMod.MOD_ID));

        //custom
        for (Map.Entry<String, Collection<Trust>> entry : TrustManager.CUSTOM_TRUST.entrySet())
            trusts.put(new TranslatableComponent(entry.getKey()), generateWidgets(container, entry.getValue(), entry.getKey()));
    }
